...
// B/C
class modLatestNewsEnhancedExtendedHelperStandard extends modLatestNewsEnhancedHelperStandard { }
...
// 					}
		...
		if (isset($array_of_authors_values['all']) && $array_of_authors_values['all'] > 0) { // 'all' was selected
			if ($params->get('allow_edit', 0)) {
				if ($user->authorise('core.edit', 'com_content')) {
					...
					$query->where('(a.state = 1) OR (a.state = 0 AND a.created_by = ' . (int) $user->get('id') . ')');
				}
			} else {
		...
		if (!$featured_only) {
			switch ($params->get('cat_order', ''))
			{
				...
				case 't_asc': $ordering .= 'c.title ASC,'; break;
				case 't_dsc': $ordering .= 'c.title DESC,'; break;
			}
		}
		...
		foreach ($items as $key => &$item) {
			...
			if (!$show_unauthorized_items || in_array($item->category_access, $authorised)) {
				$item->catlink = Route::_(ContentHelperRoute::getCategoryRoute($item->cat_slug, $item->language));
			...
			if ($params->get('allow_edit', 0)) {
				...
				if ($edit) {
					$item->link_edit = ContentHelperRoute::getArticleRoute($item->slug, $item->cat_slug, $item->language);
			...
			if ($use_intro) { // use intro text
				...
				if ($item->text) {
					if ($trigger_OnContentPrepare) { // will trigger events from plugins
						...
						JEventDispatcher::getInstance()->trigger('onContentPrepare', array('com_content.article', &$item, &$item->params, 0));
					}

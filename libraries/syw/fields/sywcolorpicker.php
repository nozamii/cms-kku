...
class JFormFieldSYWColorPicker extends JFormField
{
	...
	protected function getInput()
	{
		...
		if (!$this->allow_transparency && !$this->use_global) {
			...
			$html .= '<input style="height:auto" type="text" name="visible_'.$this->name.'" id="visible_'.$this->id.'"'.' value="'.htmlspecialchars($color, ENT_COMPAT, 'UTF-8').'"'.' class="minicolors"'.$direction.$data_rgba.$disabled.' />';
		}

...
class JFormFieldDynamicSingleSelect extends JFormFieldList
{
	...
	protected function getInput()
	{
		...
		JFactory::getDocument()->addScriptDeclaration("
			jQuery(document).ready(function () {
				...
				jQuery('#".$this->id."_elements .element.enabled').click(function() {
					jQuery('#".$this->id."_id').val(jQuery(this).attr('data-option')).change();
		...
		foreach ($options as $option) {
			...
			if (isset($option[6])) {
				$html .= '<div class="label label-' . ($option[6] === 'Pro' ? 'important' : 'warning') . ' imagelabel">' . $option[6] . '</div>';
			}
